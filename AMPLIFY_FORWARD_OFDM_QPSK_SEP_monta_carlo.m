...
%% Monta Carlo Simulations of AMPLIFY and FORWARD RELAY model with OFDM with QPSK under AWGN Noise without processing delay considerations
% Note we must normalize power at relay to that of decode and forward to
% compare the performances
% Also normalize OFDM Power every where .....
SNR = 1:2:16 ;
sep = zeros(size(SNR));
for n = 1:length(SNR)
    %% AT SOURCE 
...
%   normrcvsignalr = attinuate(rcvsignalr,sqrt(Psr)*db2mag(SNR(n)));
    symbolstreamr = normalizeenergy(rcvsignalr) ;
...
sepaf = sep ;
%figure,semilogy(SNR,sep,'*'),title('Monta Carlo Simulations of BEP of OFDM-QPSK with amplify and forward relay without processing delay considerations'),xlabel('SNR(dB)'),ylabel('BEP');
